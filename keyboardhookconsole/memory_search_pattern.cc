...
{
    for (std::string::const_iterator digit = chars.cbegin(); digit != chars.cend(); ++digit, ++pos)
    {
        if (*pos != static_cast<const unsigned char>(*digit) &&     // no match
            'X' != static_cast<const unsigned char>(*digit))        // filter out arbitrary characters
        {
            return false;
        }
        else
...
bool SearchPattern(const void* start,                  // start Address
                   ...
                   const std::string& digit,           // characteristic code
                   ...
                   void*& address)                     // return to address
{
    ...
    if (start != end && static_cast<const unsigned char*>(end) > static_cast<const unsigned char*>(start) && static_cast<std::size_t>(static_cast<const unsigned char*>(end) - static_cast<const unsigned char*>(start)) > digit.size())
    {
        std::string masks, chars;
        {
            if (StringReplace(digit, masks, " ") && StringToBinary(masks, chars, 2))
            {
                for (auto [pos, i] = std::make_tuple(static_cast<const unsigned char*>(start), static_cast<std::size_t>(0)); pos <= end; ++pos)
                {
                    if (SearchPattern(pos, chars) && ++i == index)
                    {
                        if (nullptr == address)
                        {
...
// https://www.unknowncheats.me/forum/c-and-c-/123513-patternscanner.html
int main() 
{

...
#define XORSTRING(str)      ::Mir::xor_string([]() { return str; }, std::integral_constant<std::size_t, sizeof(str) / sizeof(*str)>{}, std::make_index_sequence<::Mir::detail::_buffer_size<sizeof(str)>()>{}).crypt_get()
...
namespace Mir
{
    ...
    public:
        ...
        XORSTR_FORCEINLINE xor_string(L l, std::integral_constant<std::size_t, Size>, std::index_sequence<Indices...>) noexcept
            : _storage{ ::Mir::detail::load_from_reg((std::integral_constant<std::uint64_t, detail::load_xored_str8<Size>(Keys, Indices, l())>::value))... }
        {
        }
        ...
        XORSTR_FORCEINLINE void crypt() noexcept
        {
            ...
            alignas(alignment)
                std::uint64_t arr[]{ ::Mir::detail::load_from_reg(Keys)... };
            std::uint64_t*    keys =
                (std::uint64_t*)::Mir::detail::load_from_reg((std::uint64_t)arr);
#else
            alignas(alignment) std::uint64_t keys[]{ ::Mir::detail::load_from_reg(Keys)... };
#endif
